...
> **NOTE3:** I increased it to 64GB root + 2000GB additional volume. I ran the full test again and this time ran for longer but eventually failed with `DockerTimeoutError: Could not transition to created; timed out after waiting 4m0s` error. Some discussions about it [here](https://nextflow.slack.com/archives/C02T97HAV5M/p1688738943237399), [here](https://nextflow.slack.com/archives/C02T97HAV5M/p1696443025639869) or [here](https://nextflow.slack.com/archives/C02T97HAV5M/p1688738943237399). I tried to play around with the `ECS_CONTAINER_START_TIMEOUT` and `ECS_CONTAINER_CREATE_TIMEOUT` variables mentioned in one of these, but I reran the full test and again failed with `No space left on device errors`, despite the 2Tb additional volume. I also increased the number of IOPS to 16000 (from default 3000) when creating the AMI and that did't helped on the 500Gb AMI
